  ...
  object Internal {
    ...
    case class Source(source: Session) {
      def to(
          ...
          implicit s: Settings,
          progressSaver: ProgresSaver
        ) = {
        CassandraToCassandra(source, sessionFrom(host, port, username, password), progressSaver)
      }
    }

    case class CassandraToCassandra(from: Session, to: Session, progressSaver: ProgresSaver) {
    ...
    case class DateSource(
        cassandra: CassandraToCassandra,
        fromKeyspace: String,
        fromTable: String,
        selection: String,
        modifierAndFilter: RowData => Option[RowData]
      ) {
    ...
    case class Migration(source: DateSource, toKeyspace: String, toTable: String) {
      ...
      def run = {
        Transporter(cassandra.from, cassandra.to).process(
          Selection(fromKeyspace, fromTable, selection),
          Selection(toKeyspace, toTable),
          modifierAndFilter,
          saveResultOffset = Some(cassandra.progressSaver)
        )
      }
