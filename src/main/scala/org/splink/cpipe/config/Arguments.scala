...
class Arguments(arguments: Seq[String]) extends ScallopConf(arguments) {
  ...
  val toHosts = opt[String](required = false,default = None,
    descr = "The cassandra host ip(s). Comma-separated, if there is more then one.")

  val toKeyspace = opt[String](required = false, default = None,
    descr = "The name of the keyspace.")

  val toTable = opt[String](required = false, default = None,
    descr = "The name of the table.")

  val toUsername = opt[String](required = false, default = Some(""),
    descr = "The username for the cassandra cluster, if PasswordAuthenticator is used.")

  val toPassword = opt[String](required = false, default = Some(""),
    descr = "The password for the cassandra cluster, if PasswordAuthenticator is used.")

  val toPort = opt[Int](default = Some(Defaults.cassandraPort),
    descr = "Optional, the port2, default value is 9042.")
  ...
  val fetchSize = opt[Int](default = Some(Defaults.fetchSize),
    descr = "The amount of rows which is retrieved simultaneously. Defaults to 5000.")

  val batchSize = opt[Int](default = Some(Defaults.batchSize),
